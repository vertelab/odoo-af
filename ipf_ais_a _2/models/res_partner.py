...
class Partner(models.Model):
    ...
    def compute_ais_a_ids(self):
        ...
        _logger.info("ln48 param: %s" % param)
        ssl_params = {
        }
        ...
        for record in self:
            if record.is_jobseeker:
                _logger.info("ln152 record: %s" % record)
                ...
                _logger.info("ln48 personnummer: %s" % personnummer)
                if personnummer:
                    ...
                    for arende in res.get('arenden', []):
                        ...
                        _logger.info("ln48 param: %s" % record.ais_a_ids)
