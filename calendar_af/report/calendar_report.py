...
BASE_DAY_START = pytz.timezone(LOCAL_TZ).localize(datetime.now().replace(
    hour=9, minute=0, second=0, microsecond=0)).astimezone(pytz.utc)
BASE_DAY_STOP = pytz.timezone(LOCAL_TZ).localize(datetime.now().replace(
    hour=16, minute=0, second=0, microsecond=0)).astimezone(pytz.utc)
...
class CalendarAppointmentReport(models.Model):
    ...
    add_book_count = fields.Integer(
        string='No. additional occasions', readonly=True)
    booked_from_cal = fields.Integer(
        string='Booked from calendar', readonly=True)
    free_occ = fields.Integer(string='Free occasions', readonly=True)
    no_overbooked = fields.Integer(
        string='Overbooked occasions', readonly=True)
    app_id = fields.Integer(string='Occasion id', readonly=True)
    user_id = fields.Many2one(comodel_name='res.users',
                              string='Case worker', readonly=True)
    ...
    app_start_time = fields.Char(
        string='Appointment start time', readonly=True)
        ...
        for i in range(days_ahead):
            ...
            for j in range(16):
                for app_type in app_types:
                    search_domain = [('type_id', '=', app_type.id), ('start', '=', loop_date), (
                        'state', '=', 'ok'), ('appointment_id', '=', False)]
