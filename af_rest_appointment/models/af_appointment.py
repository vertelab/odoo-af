...
class AfAppointment(models.Model):
    ...
    def _generate_tracking_id(self, af_system_id, af_environment):
        ...
        tracking_id = "%s-%s-%s" % (af_system_id.upper(),
                                    af_environment.upper(), tracking_number)
        return tracking_id
    ...
    def get_occasions(
            ...
            appointment_type,
            max_depth=False,
            appointment_length=False,
            location_code=False,
            profession_id=False,
            employee_user_id=False):
        client_id = self.env['ir.config_parameter'].sudo(
        ).get_param('af_rest.client_id')
        client_secret = self.env['ir.config_parameter'].sudo(
        ).get_param('af_rest.client_secret')
        af_environment = self.env['ir.config_parameter'].sudo(
        ).get_param('af_rest.af_environment')
        af_port = self.env['ir.config_parameter'].sudo(
        ).get_param('af_rest.ipf_port')
        af_url = self.env['ir.config_parameter'].sudo(
        ).get_param('af_rest.ipf_url')
        af_system_id = self.env['ir.config_parameter'].sudo(
        ).get_param('af_rest.af_system_id')

        if not (
                af_url or af_port or client_id or client_secret or af_environment or af_system_id):
            raise Warning('Please setup AF integrations')

        # Generate a tracking-id
        af_tracking_id = self._generate_tracking_id(
            af_system_id, af_environment)

        # Define base url
        # ex:
        # https://ipfapi.arbetsformedlingen.se:443/appointments/v1/bookable-occasions?appointment_type=1&appointment_channel=SPD&from_date=2020-03-20&to_date=2020-03-21&client_id=da03472cd17e4ce4bb2d017156db7156&client_secret=B4BC32F21a314Cb9B48877989Cc1e3b8
        ...
        get_url = base_url.format(
            ...
            appointment_length_str=(
                "&appointment_length=%s" %
                appointment_length) if appointment_length else '',
            location_code_str=(
                "&location_code=%s" %
                location_code) if location_code else '',
            profession_id_str=(
                "&profession_id=%s" %
                profession_id) if profession_id else '',
        )
        ...
        get_headers = self._generate_headers(
            af_environment, af_system_id, af_tracking_id)
        ...
        for occasion in occasions:
            ...
            stop_datetime = datetime.strptime(
                (date + "T" + stop), "%Y-%m-%dT%H:%M")
            start_datetime = datetime.strptime(
                (date + "T" + start), "%Y-%m-%dT%H:%M")

            occ_id = occasion.get('id')
            occ = self.env['calendar.occasion'].search(
                [('ipf_id', '=', occ_id)])
            if occ:
                # Update existing 'occ'
                pass
                ...
                vals = {
                    ...
                    'duration': (stop_datetime - start_datetime).seconds // 60
    ...
    def get_appointments(
        ...
        # Define base url
        # ex:
        # https://ipfapi.arbetsformedlingen.se:443/appointments/v1/appointments?client_id=da03472cd17e4ce4bb2d017156db7156&client_secret=B4BC32F21a314Cb9B48877989Cc1e3b8&from_date=2010-10-01&pnr=199601265516
        ...
        get_url = base_url.format(
            ...
            appointment_types_str=(
                "&appointment_types=%s" %
                appointment_types) if appointment_types else '',
            # TODO: implement better, expected: comma seperated list.
            status_list_str=(
                "&status_list=%s" %
                status_list) if status_list else '',
            # TODO: implement better, expected: comma seperated list.
        )
        ...
        for appointment in appointments:
            ...
            stop_datetime = datetime.strptime(
                (date + "T" + stop), "%Y-%m-%dT%H:%M:%S")
            start_datetime = datetime.strptime(
                (date + "T" + start), "%Y-%m-%dT%H:%M:%S")

            partner = self.env['res.partner'].search(
                ['customer_id', '=', (appointment.get('customer_id'))])  # TODO: change to customer_id?
            user = self.env['res.users'].search(
                [('signature', '=', appointment.get('employee_signature'))])

            # check if appointment exists
            app = self.env['calendar.appointment'].search(
                [('ipf_id', '=', app_id)])
            if app:
                _logger.warn(
                    "DAER: app exists! app_id: %s app.channel: %s app: %s" %
                    (app.id, app.channel, app))
    ...
    def get_schedules(self, from_datetime, to_datetime, type_ids):
        ...
        get_url = base_url.format(
            ...
            client=client_id,  # check in anypoint for example
            secret=client_secret,  # check in anypoint for example
            from_date=from_datetime.strftime(
                "%Y-%m-%dT%H:%M:%SZ"),  # 2020-03-17T00:00:00Z
            to_date=to_datetime.strftime(
                "%Y-%m-%dT%H:%M:%SZ"),  # 2020-03-25T00:00:00Z
        ...
        for comp_day in req_res:
            ...
            for schedule in comp_day.get('schedules'):
                start_time = datetime.strptime(
                    schedule.get('start_time'), "%Y-%m-%dT%H:%M:%SZ")
                stop_time = datetime.strptime(
                    schedule.get('end_time'), "%Y-%m-%dT%H:%M:%SZ")

                # Integration gives us times in local (Europe/Stockholm) tz
                # Convert to UTC
                start_time_utc = pytz.timezone(LOCAL_TZ).localize(
                    start_time).astimezone(pytz.utc)
                stop_time_utc = pytz.timezone(LOCAL_TZ).localize(
                    stop_time).astimezone(pytz.utc)
                ...
                if schedule_id:
                    ...
                    vals = {
                        ...
                        'forecasted_agents': schedule.get('forecasted_agents'),
                    }
                    ...
                    vals = {
                        ...
                        'duration': 30.0,
                        # number of agents supposed to be available for this.
                        # Can sometimes be float.
                        'scheduled_agents': int(schedule.get('scheduled_agents')),
                        # May be implemented at a later date. Can sometimes be
                        # float.
