...
class account_analytic_line(models.Model):
    ...
    to_invoice = fields.Many2one(comodel_name='hr_timesheet_invoice.factor', string='Invoiceable',
        ...
        for (key_id, company_id, currency_id), analytic_lines in invoice_grouping.items():
            ...
            for analytic_line in analytic_lines:
                ...
                if not analytic_line.to_invoice:
                    raise osv.except_osv(_('Error!'), _('Trying to invoice non invoiceable line for %s.') % (
                ...
                analytic_line = self.env['account.analytic.line'].with_context(lang=partner.lang,
                                                                               force_company=company_id,
                                                                               company_id=company_id).browse(
            ...
            for (product_id, uom, user_id, factor_id, account), lines_to_invoice in invoice_lines_grouping.items():
                if separate_work_lines:
                    for line_to_invoice in lines_to_invoice:
                        curr_invoice_line = self.with_context(lang=partner.lang, force_company=company_id,
            ...
            last_invoice = self.env['account.invoice'].with_context(lang=partner.lang, force_company=company_id,
        ...
        if self[0].type == 'in_invoice':
            for il in iml:
                if il['account_analytic_id']:
                    ...
                    to_invoice = \
                    self.env['account.analytic.account'].read([il['account_analytic_id']], ['to_invoice'])[0][
                        'to_invoice']
                    if to_invoice:
...
class account_move_line(models.Model):
    ...
    def create_analytic_lines(self):
        ...
        for move_line in self:
            ...
            invoice_id = move_line.invoice_id and move_line.invoice_id.type in (
            'out_invoice', 'out_refund') and move_line.invoice_id.id or False
