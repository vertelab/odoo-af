...
class hr_timesheet_invoice_create(models.TransientModel):
    ...
    def do_create(self):
        ...
        ids = self.env['account.analytic.line'].search([('invoice_id', '=', False), ('to_invoice', '<>', False),
                                                        ('id', 'in', self._context['active_ids'])]).filtered(
